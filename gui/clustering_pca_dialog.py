...
class PCADialog(QDialog):
    def __init__(self, parent=None, default_n_components=None):
        ...
        self.default_n_components = default_n_components if default_n_components is not None else str(len(self.data_instance.get_df().columns))
        self.init_ui()
    ...
    def run_pca(self):
        try:
            ...
            if 0 < n_components <= len(self.data_instance.get_df().columns):
                pca_handler = self.data_instance.PCA(n_components)
                self.parent().display_pca_results(pca_handler)  # Wyświetl wyniki analizy PCA
                self.parent().display_pca_plot(pca_handler)  # Nowa metoda do wyświetlenia wykresu
                self.close()
            else:
                raise ValueError("Niepoprawna liczba komponentów.")
