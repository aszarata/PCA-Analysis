...
from PyQt5.QtWidgets import QPushButton, QLabel, QHBoxLayout, QComboBox, QVBoxLayout, QDialog, QLineEdit, QMessageBox, \
    QSpinBox, QWidget, QCheckBox, QFormLayout, QFileDialog, QInputDialog
...
class KMeansSuggestionDialog(QDialog):
    ...
    def run_suggestion(self):
        ...
        try:
            optimal_clusters = self.pca_handler.suggest_clusters_kmeans(max_clusters=max_clusters, draw_graph=False)
            suggested_clusters, okPressed = QInputDialog.getInt(
                self, "Sugerowana liczba klastrów", "Optymalna liczba klastrów:", optimal_clusters, 2, max_clusters, 1)

            if okPressed:
                user_decision = QMessageBox.question(self, "Uruchom KMeans",
                                                     "Czy chcesz uruchomić klastrowanie KMeans z wybraną liczbą klastrów?",
                                                     QMessageBox.Yes | QMessageBox.No)

                if user_decision == QMessageBox.Yes:
                    self.run_kmeans(
                        suggested_clusters)  # Uruchamiamy KMeans z zasugerowaną/zmodyfikowaną liczbą klastrów
            else:
                QMessageBox.information(self, "Anulowano", "Operacja została anulowana przez użytkownika.")

        except Exception as e:
            QMessageBox.critical(self, "Błąd", "Wystąpił błąd podczas obliczania sugerowanej liczby klastrów.")
    def run_kmeans(self, optimal_clusters):
        try:
            self.parent().data_instance.save()
            self.pca_handler.kmeans_clustering(num_clusters=optimal_clusters)
...
#         form_layout.addRow("Min samples -> minimalna liczba punktów, które muszą się znaleźć w klastrze", self.min_samples_input)
#
...
class DBSCANDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.pca_handler = parent.pca_handler  # Zakładamy, że PCA Handler jest dostępny w rodzicu
        self.optimal_eps = None  # Miejsce na przechowywanie sugerowanego eps
        self.init_ui()

    def init_ui(self):
        self.setWindowTitle('Grupowanie DBSCAN')
        self.setGeometry(100, 100, 300, 250)  # Zwiększenie wysokości okna
        ...
        form_layout = QFormLayout()
        self.min_samples_input = QLineEdit()
        self.eps_input = QLineEdit()  # Pole do wprowadzenia i edycji wartości eps
        self.eps_input.setEnabled(False)  # Na początku pole jest wyłączone

        self.min_samples_input.textChanged.connect(self.enable_suggestion)  # Włączenie ponownego sugerowania
        self.eps_input.setMinimumWidth(400)  # Ustawienie minimalnej szerokości pola eps_input

        form_layout.addRow("Min samples -> minimalna liczba punktów, które muszą się znaleźć w klastrze:", self.min_samples_input)
        form_layout.addRow("Optimal eps -> sugerowana wartość eps (możesz edytować):", self.eps_input)

        self.run_button = QPushButton("Zasugeruj")
        self.run_button.clicked.connect(self.run_dbscan_suggestion)

        self.apply_button = QPushButton("Zastosuj EPS i uruchom DBSCAN")
        self.apply_button.clicked.connect(self.apply_dbscan)
        self.apply_button.setEnabled(False)  # Na początku przycisk jest wyłączony

        layout.addLayout(form_layout)
        layout.addWidget(self.run_button)
        layout.addWidget(self.apply_button)

    def enable_suggestion(self):
        self.eps_input.clear()
        self.eps_input.setEnabled(False)
    ...
    def run_dbscan_suggestion(self):
        ...
        min_samples = int(min_samples)
        try:
            self.optimal_eps = self.pca_handler.suggest_clusters_dbscan(min_samples=min_samples, draw_graph=False)
            self.eps_input.setText(str(self.optimal_eps))  # Ustawienie sugerowanej wartości w polu
            self.eps_input.setEnabled(True)  # Włączenie pola do edycji
            self.apply_button.setEnabled(True)  # Włączenie przycisku zastosuj
        except Exception as e:
            QMessageBox.critical(self, "Wystąpił błąd", "Wystąpił błąd - spróbuj ponownie")

    def apply_dbscan(self):
        # Zastosowanie wartości eps z pola eps_input
        eps = self.eps_input.text()
        try:
            eps = float(eps)  # Konwersja na float, jeśli to możliwe
        except ValueError:
            QMessageBox.warning(self, "Błąd", "Wartość EPS musi być liczbą.")
            return

        min_samples = int(self.min_samples_input.text())
        self.pca_handler.dbscan_clustering(eps=eps, min_samples=min_samples)
        self.parent().dbscan_done = True
        self.display_dbscan_results()
